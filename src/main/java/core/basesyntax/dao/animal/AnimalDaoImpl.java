    ...
    public List<Animal> findByNameFirstLetter(Character character) {
        try (Session session = sessionFactory.openSession()) {
            ...
            Query<Animal> query = session.createQuery("FROM Animal "
                    + "WHERE name like :upper or name like :lower", Animal.class);
            query.setParameter("upper", Character.toUpperCase(character) + "%");
            query.setParameter("lower", Character.toLowerCase(character) + "%");
