...
public class AnimalDaoImpl extends AbstractDao implements AnimalDao {
    ...
    public List<Animal> findByNameFirstLetter(Character character) {
        try (Session session = sessionFactory.openSession()) {
            Query<Animal> findAnimalsByNameFirstLetterQuery = session.createQuery(
                    "FROM Animal a WHERE LOWER(a.name) LIKE :pattern", Animal.class);
            findAnimalsByNameFirstLetterQuery.setParameter("pattern",
                    character.toString().toLowerCase() + "%");
            return findAnimalsByNameFirstLetterQuery.getResultList();
        } catch (Exception e) {
            throw new RuntimeException("Couldn't get animals whose names starts with "
                    + character, e);
        }
    }
}
