    ...
    public List<Animal> findByNameFirstLetter(Character character) {
        try (Session session = sessionFactory.openSession()) {
            ...
            Predicate lowerPredicate =
                    cb.like(root.get("name"),String.valueOf(character).toLowerCase() + "%");
            Predicate upperPredicate
                    = cb.like(root.get("name"),String.valueOf(character).toUpperCase() + "%");
            query.where(cb.or(lowerPredicate, upperPredicate));
