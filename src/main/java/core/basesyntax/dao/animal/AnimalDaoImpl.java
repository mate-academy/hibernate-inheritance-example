    ...
    public List<Animal> findByNameFirstLetter(Character character) {
        try (Session session = sessionFactory.openSession()) {
            Query<Animal> animalQuery = session.createQuery("from Animal a "
                    + "where upper(a.name) like :firstLatter", Animal.class);
            animalQuery.setParameter("firstLatter",
                    character.toString().toUpperCase() + "%");
            return animalQuery.getResultList();
        } catch (Exception e) {
            throw new RuntimeException("Can't get animal by first latter " + character, e);
        }
    }
}
